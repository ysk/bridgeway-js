...
覚えるのは `$` を `$$` にすることだけ。**モダンな技術（Svelte、将来はVueなど）を、限りなく楽に** ＝ 学習コストの削減。

jQueryの書き味（`$$` / `state` / `mount`）はそのまま。その裏で動くのは「なんちゃって」ではなく **本物のフレームワーク**。DOMの差分更新・リアクティビティ・後片付けは、実績あるエンジンに丸ごと委ねます。

**もう新しいフロントエンドフレームワークに翻弄されない** — エンジンは差し替え式。利用者が書く `$$ / state / mount` は、下がSvelteでもVueでも変わりません。

---

## いまの立ち位置（正直版）

この製品は方針転換しました。以前の「script1本・ビルド不要・自前テンプレ」路線はやめています。理由は、自前実装（擬似テンプレ＋擬似リアクティビティ）ではモダンの旨味が出ず、むしろ地雷（フォーカス飛び・CSP違反・メモリリーク）を抱えていたためです。

- ✅ **本物のSvelteに委譲** — 構造・`{#if}`/`{#each}`・DOM差分は `.svelte` が担当（外科手術的更新）
- ✅ **CSPセーフ** — `eval`(`new Function`) を使わない。`script-src 'self'` でも動く
- ✅ **自動後片付け** — `mount().destroy()` でSvelteが購読もDOMも破棄（リークしない）
- ✅ **エンジン差し替え式** — 将来 `vue` エンジンを足すだけ。利用者コードは不変
- ⚠️ **ビルドが必要** — 本物のSvelte＝コンパイラ前提。`.svelte` をバンドルします
- ⚠️ **jQuery共存は“サポートしない”** — 機能として保証・お膳立てはしません（下記）
...
```js
// main.js
import { $$, state, mount } from "bridgeway";   // ← 開発中は "../../src/index.js"
import App from "./App.svelte";

const app = mount(App, { target: "#app", props: { name: "bridgeway" } });
...
| URL | 内容 |
| --- | --- |
| http://localhost:8000/sample/deep/ | 深いSvelte移行の最小デモ（カウンタ／`{#if}`／破棄。CSP有効・入力フォーカス保持） |
| http://localhost:8000/sample/todoapp/ | ToDo（keyed `{#each}` の差分更新。`$$` グルーで外のバッジ更新） |
| http://localhost:8000/sample/vue/ | **同じ画面をVueエンジンで**。利用者コードは `sample/deep` とほぼ同一（`useEngine(vueEngine)` を足すだけ） |
...
```bash
npm install bridgeway
npm install svelte   # または vue（使う方だけ。optional peer）
```

**② `<script>` 一本（ビルド不要・CDN）** — Vue公式のCDNフルビルドと同じ発想。

```html
<!-- Vueフルビルド同梱。mount まで全部動く -->
<script src="dist/bridge.vue.js"></script>
<script>
...
- `dist/bridge.vue.js` … Vueフルビルド同梱。ビルド不要で template 文字列の部品を `mount` まで。
- `dist/bridge.js` … Svelte(軽量)同梱。`$$ / state / computed` 中心。
- グローバル版は `npm run build`（`build.mjs`）で `dist/` に生成。npm公開後は `https://cdn.jsdelivr.net/npm/bridgeway/dist/bridge.vue.js` で読めます。
...
- `src/mount.js` … `mount()` は現在のエンジンの `mount` へ**委譲**（Svelte決め打ちにしない）
- 将来ビジョン: `npm install bridgeway` → `select framework: ◯ svelte ◯ vue` でエンジンを自動配線

配布用グローバルビルド（`<script>`向け）は `npm run build`（`build.mjs`）で `bridgeway` を再生成します。ただし `mount` はコンパイル済みコンポーネントを渡す前提なので、実運用では利用者側のビルドと併用します。
...
bridgeway は [MIT License](./LICENSE)。

配布ビルド（`dist/`）には Svelte / Vue（いずれもMIT）が同梱されます。各ライセンス表記は [THIRD-PARTY-NOTICES.md](./THIRD-PARTY-NOTICES.md) を参照してください。

> bridgeway は独立した**非公式**プロジェクトで、jQuery / Svelte / Vue の各プロジェクト・団体とは提携・後援・承認の関係にありません。jQuery の**書き味に着想**を得ていますが、jQuery のコードは含まず、APIを独自に再実装したものです。各名称は権利者の商標です。
