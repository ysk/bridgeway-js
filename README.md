...
## jQueryライクな書き味でできる、モダン開発。

覚えるのは `$` を `$$` にすることだけ。モダンな技術（Svelte・Vue）を、限りなく楽に。

jQueryの書き味（`$$` / `state` / `mount`）はそのまま。その裏で動くのは**Svelt**や**Vue**などのモダンフレームワーク。DOMの差分更新・リアクティビティ・後片付けは、実績あるエンジンに丸ごと委ねます。
