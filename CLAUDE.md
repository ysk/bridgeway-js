jQueryライクな書き味を残しつつ、本物のモダンフレームワーク（Svelte、Vue）の恩恵を、限りなく楽に受けられるOSS。
...
・究極像: 「npm install bridgey → フレームワーク選択(svelte / vue)」でエンジンを対話的に選び自動配線。
...
・src/bridgey.js … $$（命令的グルー）＋ state/computed（束縛はdispose()で購読解除）
